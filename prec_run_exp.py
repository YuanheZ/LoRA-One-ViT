...
def reinit_lora_modules(name, module, init_config, **kwargs):
    ...
    if init_config.mode == "simple":
        ...
        if init_config.direction == "OS-LoRA-Full-AN":
            ...
            eta_scale = kwargs["eta"]
            print("Using eta value", eta_scale)
            gamma = torch.tensor(eta_scale)
            B = torch.sqrt(gamma) * U[:, :lora_r] @ torch.diag(torch.sqrt(S[:lora_r])) #/ torch.sqrt(S[0])
            A = torch.sqrt(gamma) * torch.diag(torch.sqrt(S[:lora_r])) @ V[:lora_r, :] #/ torch.sqrt(S[0])
...
def run_exp(cfg: DictConfig):
    ...
    if use_peft and cfg.peft.get("dora", False):
        ...
        if cfg.init.mode == "gradient":
           peft_config = LoraConfig(
               r=lora_r,
               lora_alpha=cfg.peft.lora_alpha, # cancel square root of lora rank
               target_modules=lora_target_modules,
               use_rslora=if_use_rslora,
           )
        else:
