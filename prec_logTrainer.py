...
@dataclass
class PrecTrainingArguments(Seq2SeqTrainingArguments):
    prec_reg: Optional[float] = field(
        default=1e-6, metadata={"help": "regularization parameter for the preconditioners."}
    )
...
class LogTrainer(Trainer):
    def __init__(
        ...
        args: PrecTrainingArguments = None,
        ...
        assert isinstance(
            args, PrecTrainingArguments
        ), "args must be of type PrecTrainingArguments"
        super().__init__(
            model,
            args,
            data_collator,
    ...
    def create_optimizer(self):
        ...
        """
        prec_reg = getattr(self.args, "prec_reg", None)
        ...
        if self.optimizer is None:
            ...
            #optimizer_kwargs.update({'optimizer_reg': self.prec_reg})
            self.optimizer = AdamWr(optimizer_grouped_parameters, optimizer_reg=prec_reg, **optimizer_kwargs)
